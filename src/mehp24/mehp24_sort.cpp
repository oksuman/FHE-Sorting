...
)
{
    ...
    for (size_t i = 0; i < numReqThreads; i++) {
        ...
        Ciphertext<DCRTPoly> Cjk = comp->compare(m_cc, replR[j], replC[k], SignFunc, Cfg);
    ...
    CryptoContext<DCRTPoly> cc)
{
    ...
    std::vector<Ciphertext<DCRTPoly>> sortedParts = sortFG(parts, subLength, dg_c, df_c, dg_i, df_i);
