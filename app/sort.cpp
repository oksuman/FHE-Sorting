...
arraySort::arraySort(std::string ccLocation, 
...
void arraySort::initCC()
{
    ...
    if (!Serial::DeserializeFromFile(m_arrayLocation, input_array, SerType::BINARY))
    {
...
void arraySort::eval()
{
    ...
    auto rotIndex = m_cc->EvalChebyshevFunction(scaled_sinc, Index_minus_Rank, -1, 1, sincPolyDegree);
    ...
    for(int i=0; i<64; i++){
        ...
        if(i==0){
            ...
            for(int j=1; j<32; j++){
                ...
                auto rotIndex = m_cc->EvalChebyshevFunction( [j](double x) { return scaled_sinc_j(x, j); }, Index_minus_Rank, -1, 1, 7701);
                auto tmp =  m_cc->EvalMultAndRelinearize(rotIndex, input_array);

                m_cc->EvalAddInPlace(intermediate, m_cc->EvalRotate(tmp, j-32));
            }
            ...
            for(int j=0; j<32; j++){
                ...
                auto rotIndex = m_cc->EvalChebyshevFunction([i,j](double x) { return scaled_sinc_j(x, i*32+j); }, Index_minus_Rank, -1, 1, 7701);
                auto tmp = m_cc->EvalMultAndRelinearize(rotIndex, input_array);
...
{
    if (!Serial::SerializeToFile(m_OutputLocation, output_array, SerType::BINARY))
    {
...
double arraySort::scaled_sinc(double x) {
    if (std::abs(x) < 1e-10) {
        return 1.0;
    } else {
...
double arraySort::scaled_sinc_j(double x, int j) {
    ...
    double term1 = (std::abs(denominator1) < epsilon) ? 1.0 : std::sin(denominator1) / denominator1;
    double term2 = (std::abs(denominator2) < epsilon) ? 1.0 : std::sin(denominator2) / denominator2;
