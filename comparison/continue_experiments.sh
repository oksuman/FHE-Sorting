...
START_TRIAL_KWAY_K5=4
START_TRIAL_OURS=1
START_TRIAL_MEHP24=1
START_TRIAL_OURS_HYBRID=1
...
run_test_continued() {
    ...
    echo "Running $algo tests from trial $start_trial to $NUM_TRIALS"
    cd "$TEST_DIR" || exit 1
    
    for trial in $(seq $start_trial $NUM_TRIALS); do
...
if [ $START_TRIAL_KWAY_K5 -le $NUM_TRIALS ]; then
    run_test_continued "kway_k5" "k-way/KWaySort5Test" $START_TRIAL_KWAY_K5 "${KWAY_K5_SIZES[@]}"
    sync
    sleep 30
fi

if [ $START_TRIAL_OURS -le $NUM_TRIALS ]; then
    run_test_continued "ours" "DirectSortTest" $START_TRIAL_OURS "${DIRECT_SIZES[@]}"
    sync
    sleep 30
fi

if [ $START_TRIAL_MEHP24 -le $NUM_TRIALS ]; then
    run_test_continued "mehp24" "mehp24/Mehp24SortTest" $START_TRIAL_MEHP24 "${MEHP24_SIZES[@]}"
    sync
    sleep 30
fi

if [ $START_TRIAL_OURS_HYBRID -le $NUM_TRIALS ]; then
    run_test_continued "ours_hybrid" "DirectSortHTest" $START_TRIAL_OURS_HYBRID "${DIRECT_SIZES[@]}"
    sync
    sleep 30
fi
